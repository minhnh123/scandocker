...
def scan_image(image_name: str):
    ...
    try:
        ...
        cmd = [
            ...
            "image", "--format", "json", "--no-progress", image_name
        ]
...
def scan_cis():
    ...
    try:
        cmd = [
            "docker", "run", "--rm", 
            "--network", "host", "--pid", "host", "--userns", "host", "--cap-add", "audit_control",
...
def auto_remediate(request: RemediationRequest):
    try:
...
@app.get("/api/history")
def get_scan_history(image_name: str = None):
    try:
