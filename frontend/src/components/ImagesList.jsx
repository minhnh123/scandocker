...
function ImagesList({ images }) {
  ...
  const handleRemediate = async (vuln, osContext) => {
    ...
    try {
      ...
      setRemediationError({ id: vuln.VulnerabilityID, message: "Lỗi phản hồi từ AI: " + (err.response?.data?.detail || err.message) });
    } finally {
      setRemediating(null);
    }
  };
  ...
  const handleExportCSV = (res) => {
    ...
    const rows = res.Vulnerabilities.map(v => 
  ...
  const handleScan = async (imageTag) => {
    ...
    setScanResult(null);
    try {
      const res = await axios.get(`http://localhost:8002/api/scan/image?image_name=${encodeURIComponent(imageTag)}`);
    ...
    return (
      <div className="glass-panel" style={{ padding: '24px' }}>
        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
          <button className="btn btn-primary" onClick={() => setScanResult(null)}>Quay lại danh sách</button>
          <button className="btn btn-primary" style={{ background: '#0088cc', color: 'white' }} onClick={handleTestAlert} disabled={alerting}>
        ...
        {scanResult?.Results ? scanResult.Results.map((res, i) => (
          <div key={i} style={{ marginTop: '20px', padding: '16px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3>{res.Target} ({res.Type})</h3>
              {res.Vulnerabilities && (
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="btn btn-primary" style={{ padding: '6px 12px', fontSize: '13px', background: 'var(--success)' }} onClick={() => handleExportPDF(res)}><Download size={14} style={{marginRight: '4px'}}/> Tải PDF</button>
                  <button className="btn btn-primary" style={{ padding: '6px 12px', fontSize: '13px', background: 'var(--warning)', color: 'black' }} onClick={() => handleExportCSV(res)}><Download size={14} style={{marginRight: '4px'}}/> Xuất CSV</button>
                </div>
              )}
            </div>
            {res.Vulnerabilities ? (
                ...
                <tbody>
                  {res.Vulnerabilities.map((vuln, j) => (
                    <tr key={j} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '8px', color: 'var(--primary)' }}><a href={vuln.PrimaryURL} target="_blank" rel="noreferrer" style={{color: 'inherit'}}>{vuln.VulnerabilityID}</a></td>
                      <td style={{ padding: '8px', color: vuln.Severity === 'CRITICAL' ? 'var(--danger)' : vuln.Severity === 'HIGH' ? 'var(--warning)' : 'var(--text-main)' }}>{vuln.Severity}</td>
                      <td style={{ padding: '8px' }}>{vuln.PkgName}</td>
                      <td style={{ padding: '8px' }}>{vuln.InstalledVersion}</td>
                      <td style={{ padding: '8px', color: 'var(--success)' }}>{vuln.FixedVersion || 'N/A'}</td>
                      <td style={{ padding: '8px' }}>
                        <button 
                          className="btn btn-primary" 
                          style={{ padding: '4px 8px', fontSize: '12px', background: 'var(--success)' }}
                          onClick={() => handleRemediate(vuln, res.Target)}
                          disabled={remediating === vuln.VulnerabilityID}
                        >
                          {remediating === vuln.VulnerabilityID ? 'Đang phân tích...' : <><Wand2 size={12} style={{marginRight: '4px'}}/> Vá lỗi (AI)</>}
                        </button>
                      </td>
                    </tr>
                  ))}
              ...
              <p style={{ marginTop: '12px', color: 'var(--success)' }}>Không tìm thấy lỗ hổng nào!</p>
            )}
          </div>
        )) : <p>Không có dữ liệu phân tích chi tiết.</p>}
        
        {/* AI Remediation Modal */}
        {(remediationData || remediationError) && createPortal(
          <div style={{
            position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, 
            background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)'
          }}>
            <div style={{
              background: 'var(--bg-dark)', width: '800px', maxWidth: '90vw', maxHeight: '85vh', 
              borderRadius: '12px', border: `1px solid ${remediationError ? 'var(--danger)' : 'var(--success)'}`,
              padding: '24px', display: 'flex', flexDirection: 'column',
              boxShadow: '0 20px 40px rgba(0,0,0,0.6)', overflowY: 'auto'
            }}>
              {remediationError ? (
                <>
                  <h3 style={{ color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                    <ShieldAlert size={24} /> Lỗi sinh mã vá cho {remediationError.id}
                  </h3>
                  <p style={{ color: 'var(--text-main)', background: 'rgba(239, 68, 68, 0.1)', padding: '16px', borderRadius: '8px' }}>{remediationError.message}</p>
                  <button className="btn btn-primary" onClick={() => setRemediationError(null)} style={{marginTop: '24px', background: 'var(--danger)', border: 'none', alignSelf: 'flex-end'}}>Đóng thông báo</button>
                </>
              ) : (
                <>
                  ...
                  <button className="btn btn-primary" onClick={() => setRemediationData(null)} style={{marginTop: '24px', alignSelf: 'flex-end'}}>Đóng gợi ý</button>
                </>
              )}
            </div>
          </div>,
          ...
          {images.map(img => (
            <tr key={img.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <td style={{ padding: '12px' }}>
                ...
                >
                  {scanning === img.tags[0] ? 'Đang quét...' : <><ShieldAlert size={16} /> Quét bảo mật</>}
                </button>
              </td>
            </tr>
          ))}
          {images.length === 0 && (
            <tr><td colSpan="4" style={{ padding: '16px', textAlign: 'center' }}>Không có Image nào.</td></tr>
          )}
        </tbody>
      </table>
      </div>
