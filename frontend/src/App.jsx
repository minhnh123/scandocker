...
import { Shield, LayoutDashboard, Box, HardDrive, Settings, AlertTriangle, Activity } from 'lucide-react';
...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!rootRes.data.docker_available) {
        setDockerError("Docker Desktop hiện chưa được bật. Vui lòng khởi động Docker trên máy chủ của bạn.");
        setLoading(false);
        return;
      }
  ...
  const renderContent = () => {
    if (loading) {
        return <div style={{display: 'flex', justifyContent: 'center', marginTop: '100px'}}><div className="spin"><Shield size={48} color="var(--primary)" /></div></div>;
    }
    ...
    return (
        <div>
          <div className="dashboard-grid">
            <div className="stat-card glass-panel" onClick={() => setActiveView('images')} style={{cursor: 'pointer'}}>
              <div className="stat-title">Tổng số Images</div>
              <div className="stat-value">{stats.images.length}</div>
            </div>
            <div className="stat-card glass-panel" onClick={() => setActiveView('containers')} style={{cursor: 'pointer'}}>
              <div className="stat-title">Containers Đang chạy</div>
              <div className="stat-value" style={{color: 'var(--success)'}}>{stats.containers.filter(c => c.status === 'running').length}</div>
            </div>
            <div className="stat-card glass-panel">
              <div className="stat-title">Lượt quét (History)</div>
              <div className="stat-value" style={{ color: 'var(--info)' }}>{history.length}</div>
            </div>
          </div>
              ...
              <div style={{ width: '100%', height: 350 }}>
                <ResponsiveContainer>
                  <LineChart data={history} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    ...
                    <XAxis dataKey="timestamp" stroke="#888" tickFormatter={(tick) => new Date(tick).toLocaleDateString()} />
                    <YAxis stroke="#888" />
                    <Tooltip 
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="critical" stroke="#ff4d4f" strokeWidth={3} name="Critical" dot={{r: 4}} activeDot={{r: 6}} />
                    <Line type="monotone" dataKey="high" stroke="#faad14" strokeWidth={2} name="High" />
                    <Line type="monotone" dataKey="medium" stroke="#1890ff" strokeWidth={2} name="Medium" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
  ...
  return (
    <div className="app-container">
      ...
      <div className="sidebar glass-panel" style={{ borderRadius: 0, borderTop: 0, borderBottom: 0, borderLeft: 0 }}>
        ...
        <a href="#" className={`nav-item ${activeView === 'images' ? 'active' : ''}`} onClick={(e) => { e.preventDefault(); setActiveView('images'); }}>
          <Box size={20} /> Danh sách Images
        </a>
        <a href="#" className={`nav-item ${activeView === 'containers' ? 'active' : ''}`} onClick={(e) => { e.preventDefault(); setActiveView('containers'); }}>
          <HardDrive size={20} /> Danh sách Containers
        </a>
        <a href="#" className={`nav-item ${activeView === 'cis' ? 'active' : ''}`} onClick={(e) => { e.preventDefault(); setActiveView('cis'); }}>
          <Settings size={20} /> CIS Benchmark
        </a>
        
        <div style={{marginTop: 'auto', marginBottom: '24px'}}>
             <button className="btn btn-primary" style={{width: '100%', justifyContent: 'center'}} onClick={fetchData}>
                 Làm mới dữ liệu
             </button>
        </div>
      </div>
      ...
      <div className="main-content">
        ...
        {dockerError && (
          <div className="glass-panel" style={{ padding: '16px 24px', marginBottom: '24px', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderLeft: '4px solid var(--danger)' }}>
